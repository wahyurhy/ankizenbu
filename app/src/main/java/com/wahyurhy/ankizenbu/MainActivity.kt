                    ...
                    TopAppBar(
                        title = {
                            Row(
                                ...
                                val showCategoryWords = remember { mutableStateOf<Pair<Int, List<String>>?>(null) }

                                // Klik 1x
                                ClickableText(
                                    text = AnnotatedString("Klik 1x: ${totalClicksByCount.value[1] ?: 0}"),
                                    style = TextStyle(
                                        fontSize = 16.sp,
                                        color = Color(0xFFBB0000) // Merah cerah
                                    ),
                                    onClick = {
                                        val categoryWords = sharedPreferences.all
                                            .filter { it.key.endsWith("_clickCount") && it.value == 1 }
                                            .keys.map { it.removeSuffix("_clickCount") }
                                        showCategoryWords.value = 1 to categoryWords
                                    }
                                )

                                // Klik 2x
                                ClickableText(
                                    text = AnnotatedString("Klik 2x: ${totalClicksByCount.value[2] ?: 0}"),
                                    style = TextStyle(
                                        fontSize = 16.sp,
                                        color = Color(0xFF60D500) // Hijau cerah
                                    ),
                                    onClick = {
                                        val categoryWords = sharedPreferences.all
                                            .filter { it.key.endsWith("_clickCount") && it.value == 2 }
                                            .keys.map { it.removeSuffix("_clickCount") }
                                        showCategoryWords.value = 2 to categoryWords
                                    }
                                )

                                // Klik 3x
                                ClickableText(
                                    text = AnnotatedString("Klik 3x: ${totalClicksByCount.value[3] ?: 0}"),
                                    style = TextStyle(
                                        fontSize = 16.sp,
                                        color = Color(0xFF447446) // Hijau tua
                                    ),
                                    onClick = {
                                        val categoryWords = sharedPreferences.all
                                            .filter { it.key.endsWith("_clickCount") && it.value == 3 }
                                            .keys.map { it.removeSuffix("_clickCount") }
                                        showCategoryWords.value = 3 to categoryWords
                                    }
                                )

                                // Dialog untuk menampilkan kata-kata dalam kategori
                                showCategoryWords.value?.let { (category, words) ->
                                    var searchText by remember { mutableStateOf("") } // State untuk teks pencarian
                                    val filteredWords = words.filter { it.contains(searchText, ignoreCase = true) } // Filter kata berdasarkan pencarian

                                    AlertDialog(
                                        onDismissRequest = { showCategoryWords.value = null },
                                        title = {
                                            Text("Kata-kata Klik ${category}x")
                                        },
                                        text = {
                                            Column(
                                                modifier = Modifier
                                                ...
                                                TextField(
                                                    ...
                                                    placeholder = { Text("Cari kata...") },
                                                    modifier = Modifier
                                                        .fillMaxWidth()
                                                        .padding(bottom = 16.dp)
                                                )

                                                // Daftar Kata yang Dapat Diklik
                                                LazyColumn(
                                                    modifier = Modifier
                                                        .fillMaxWidth()
                                        ...
                                        confirmButton = {
                                            Button(onClick = { showCategoryWords.value = null }) {
                                                Text("Tutup")
                                            }
                                        }
                                    )
